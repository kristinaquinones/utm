...
- **Tenant isolation:** Data access goes through a `Store` bound to one `user_id` (`app/repository.py`). Never add an un-scoped query path; every read and write is filtered to a single user. Normal routes derive that user from the session. The **admin moderation** routes (behind `require_admin`) may bind the `Store` to another user's id to audit or take down content, and those cross-tenant actions are recorded in `admin_actions`. For non-admins, cross-tenant access must always fail closed (404 / no-op).
